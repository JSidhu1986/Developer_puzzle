...
export class PriceQueryFacade {
  ...
  fetchQuote(symbol: string, period: string, selectedFromDate?: string, selectedToDate?: string) {
    this.store.dispatch(new FetchPriceQuery(symbol, period, selectedFromDate, selectedToDate));
  }
}
